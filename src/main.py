...
app = FastAPI(title="Syndicate Transcript Backend", version="1.0.0")
...
async def http_exception_handler(request, exc):
    # detail is usually a plain string, but handlers may raise
    # HTTPException(detail={"message": ..., "data": ...}) when the error
    # response needs to carry structured data (e.g. a pending verification
    # token) alongside the message.
